...
extern volatile unsigned char _data;    // for mark data not executable
extern volatile unsigned char _end;    // end of physical our memory

// Set up page translation tables and enable virtual memory(MMU)
static void mmuInit() {
    unsigned long *paging = (unsigned long *) &_end;
    unsigned long r, b;

    // Create MMU translation tables at endMemory

    // TTBR0, identity L1
    paging[0]=(unsigned long)((unsigned char*)&_end+2*PAGE_SIZE) |    // physical address
    ...
    for(r=0;r<512;r++) {
        paging[3 * 512 + r] = (unsigned long) (r * PAGE_SIZE) |   // physical address
                              ...
                              PT_USER |     // non-privileged
                              PT_ISH |      // inner shareable
                              ((r < 0x80 || r > (unsigned long) &_data / PAGE_SIZE) ? PT_RW | PT_NX
                                                                                           : PT_RO); // different for code and data
    }

    // TTBR1, kernel L1
    paging[512+511]=(unsigned long)((unsigned char*)&_end+4*PAGE_SIZE) | // physical address
                    PT_PAGE |     // we have area in it mapped by pages
                    PT_AF |       // accessed flag
    ...
    // kernel L2
    paging[4*512+511]=(unsigned long)((unsigned char*)&_end+5*PAGE_SIZE) |   // physical address
                      PT_PAGE |     // we have area in it mapped by pages
                      PT_AF |       // accessed flag
                      PT_KERNEL |   // privileged
                      PT_ISH |      // inner shareable
    ...
    // kernel L3
    paging[5*512]=(unsigned long)(MMIO_BASE_ADDRESS+0x00201000) |   // physical address
                  PT_PAGE |     // map 4k
                  PT_AF |       // accessed flag
                  PT_NX |       // no execute
                  PT_KERNEL |   // privileged
    ...
    if(r&(0xF<<28)/*4k*/ || b<1/*36 bits*/) {
        UART0puts("ERROR: 4k granule or 36 bit address space not supported\n");
        UART0puts("YoYo halted!\n");

        printDebug("ERROR: 4k granule or 36 bit address space not supported\n");
        printDebug("YoYo halted!\n");
        for(;;)
            ;
    }
    ...
    asm volatile ("msr ttbr1_el1, %0" : : "r" ((unsigned long)&_end + TTBR_ENABLE + PAGE_SIZE));
